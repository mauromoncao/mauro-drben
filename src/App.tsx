...
import {
  ...
  ChevronRight, Zap, Building2, Shield, MessageSquare
} from 'lucide-react'
...
const navGroups = [
  {
    label: 'VISÃO GERAL',
    items: [
      { to: '/', icon: Building2, label: 'HUB Comercial', exact: true },
    ],
  },
  {
    ...
    <aside className="w-64 bg-primary-900 min-h-screen flex flex-col fixed left-0 top-0 z-40 overflow-y-auto">
      ...
      <div className="p-5 border-b border-primary-700 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-white font-bold text-base leading-tight">Lex OS</h1>
            <p className="text-primary-200 text-xs">Gestão Comercial</p>
          </div>
        </div>
      </div>
    ...
    <header className="h-14 bg-white border-b border-slate-200 fixed top-0 right-0 left-64 z-30 flex items-center justify-between px-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1.5 bg-green-50 border border-green-200 rounded-full px-3 py-1.5">
          ...
          <span className="text-green-700 text-xs font-medium">4 Agentes Ativos</span>
        </div>
        {/* Notificações */}
